...
class Game():
	...
	def startScreen(self, render):
		...
		self.foodInfo = self.snake.foodPos
	...
	def nextFrame(self, keyPressed, render=False):
		...
		if (tuple(self.snake.head.dir) == (-1 * dirn[0], -1 * dirn[1])) and self.snake.n > 0:
				self.reward = -2
				return -1
		x, y = self.snake.getPos()        
		self.snake.move()
		newX, newY = self.snake.getPos()
		foodX, foodY = self.snake.foodPos
        
		if ((abs(newX-foodX) < abs(x-foodX)) or (abs(newY-foodY) < abs(y-foodY))):
				self.reward = self.snake.n    
		else:
				self.reward = -1
	...
	def snakeInfo(self):
		...
		info.append(self.foodInfo[0])
		info.append(self.foodInfo[1])
		info.append(list(map(lambda x: x.pos(), self.snake.tail[1:])))
# 		reward = (700 - ((info[0]-info[2])**2 + (info[1]-info[3])**2)**0.5) / 700
		return info, self.reward
